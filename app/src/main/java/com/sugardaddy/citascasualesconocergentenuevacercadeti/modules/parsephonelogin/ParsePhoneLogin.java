package com.sugardaddy.citascasualesconocergentenuevacercadeti.modules.parsephonelogin;
...
public class ParsePhoneLogin {
         ...
         synchronized (lock) {
             if (!isInitialized) {
                 throw new IllegalStateException(
                         "You must call ParsePhoneLogin.initialize() before using ParsePhoneLogin");
             }
         }
     }
    ...
    /*public static Task<Void> linkInBackground(ParseUser user, GoogleSignInAccount account) {
      ...
      *
      * @param id The Firebase user which passes along the result via
      * @param token The Firebase user which passes along the result via
      * @param callback The [LogInCallback] which is invoked on log in success or error
      */

   public static void LoginInBackGround(String id, String token, LogInCallback callback){

       checkInitialization();

       mCurrentCallback = callback;

       LoginInBackground(id, token);

   }

    public static Map<String, String> getAuthData(String id, String token){

        Map<String, String> authData = new HashMap<>();
        authData.put("id", id);
        authData.put("id_token", token);

        return authData;
    }

    public static void LoginInBackground(String id, String token){

        Log.d("GOOGLE_LOGIN", "Logging Parse Server account");

        ParseUser.logInWithInBackground(AUTH_TYPE, getAuthData(id, token)).continueWith(task -> {
            if (task.isCompleted()){
